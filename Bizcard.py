...
create_query1= ('''CREATE TABLE IF NOT EXISTS Bizcard.card_data
...
def create_df(data):
    ...
    if not all(length == lengths[0] for length in lengths):
         ...
         for key, values in data.items():                                       #revise this function without fail
            if len(values) == 0:
                data[key] = [''] * max_length
           ...
           # else: 
                #data[key] += [''] * (max_length - len(values))
...
st.write(df)                                                                   #need to work on the mobile number extracting issue
  
 
                                                                                   
if st.button("upload to Database"):                                               
     for i, row in df.iterrows():
          insert_query =  '''INSERT INTO card_data(company_name,card_holder,designation,mobile_number,email,website,area,city,state,pin_code,image)
...
if st.button ("View updated data"):
     ...
     st.write(updated_df)  
     st.success("### uploaded to database sucessfully")
    ...
    st.subheader(':blue[You can View ,  Delete the extracted data in Modify option]')
